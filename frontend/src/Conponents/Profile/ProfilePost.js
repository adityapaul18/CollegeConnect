...
function HomePost({ setopen2,fetchPosts, post, setModal,savedPosts, setSavedPosts,fetchSavedPosts,editModal,setEditModal,editData,setEditData}) {
        ...
        <div className="ProfilePost" >
            <div className="PostTop">
                ...
                {userID==post.question.user._id&&<TextField value=":" className="optionMenu" select>
                    ...
                    <MenuItem value="Delete" onClick={async(e)=>{
                      e.preventDefault();
                      Swal.fire({
            ...
            <div className="PostAnswer">
                ...
                <div className="ShowAnswers" > {token && <span onClick={() => { setopen2(1); setModal(post) }}>Write Answer</span>} <span onClick={() => history.push({ pathname: '/answers', state: post._id })}> Show Answers </span>{token?savedPosts.filter((s)=>s._id==post._id)[0]?<BookmarkIcon
                  onClick={async(e)=>{
                    e.preventDefault();
